...
export function UserManagementPage({ role = 'superadmin' }: UserManagementPageProps) {
  ...
  return (
    <div className="space-y-6 w-full pb-16 font-sans text-slate-800">
      {/* VIEW-ONLY BANNER FOR ADMIN AND NON-SUPERADMIN/DEVTEAM ROLES */}
      {!canModifyUsers && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-amber-800 font-medium">
                Admin users have view-only access to user management. Creating users, editing roles, resetting passwords, and deleting accounts are restricted to <strong>Super Admin</strong> and <strong>Dev Team</strong>.
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {canModifyUsers ? (
            ...
            <span className="px-3.5 py-2 bg-slate-100 text-slate-500 border border-slate-200 text-xs font-bold rounded-xl flex items-center gap-1.5 cursor-not-allowed">
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-700 font-medium">
              {filteredUsers.map(user => (
                <tr key={user.id} className="hover:bg-purple-50/30 transition-colors">
                  ...
                  {/* Column 4: Actions (RESTRICTED IF NOT SUPERADMIN OR DEVTEAM) */}
                  <td className="py-4 px-4 text-right whitespace-nowrap space-x-2">
                    {canModifyUsers ? (
                      <>
                        {/* ASSIGN ROLE / EDIT ROLE BUTTON */}
                        <button
